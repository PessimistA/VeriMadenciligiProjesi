...
TARGET_PATH = '/content/Dental_Veri/YOLO/Dental_Veri_V4_HighRes'
...
print("1. AŞAMA: Veri Ön İşleme Başlıyor (CLAHE & Güvenli Kırpma)...")
...
yaml_save_path = f'{TARGET_PATH}/dental_data_v4.yaml'
with open(yaml_save_path, 'w') as f:
    yaml.dump(data_config, f, default_flow_style=False, sort_keys=False)


print("\n3. AŞAMA: YOLO11 Medium Eğitimi Başlıyor (Pro Mod)...")
model = YOLO('yolo11m.pt')

zaman = datetime.now().strftime("%Y%m%d_%H%M%S")
guvenli_kayit_yolu = f'/content/drive/MyDrive/Veri_Madenciligi/YOLO_Canli_Kayit/Proje_{zaman}'
os.makedirs(guvenli_kayit_yolu, exist_ok=True)
...
results = model.train(
    ...
    workers=8,          
    patience=10,
    project=guvenli_kayit_yolu,
    name='v11m_high_res_pro'
)

print(f"\n✅ Eğitim süreci tamamlandı. Tüm ağırlıklar ve grafikler şu dizinde güvendedir:\n{guvenli_kayit_yolu}/v11m_high_res_pro")